...
public class GeneradorPDFFacturaLocalNormal extends GeneradorPDFFactura {
    ...
    public GeneradorPDFFacturaLocalNormal(Arrendatario arrendatario, Inmueble inmueble, Contrato contrato, Configuracion configuracion, Boolean version) {
    ...
    private void rellenarContenido(LocalDate fechaFactura) throws DocumentException {
        ...
        if (arrendatario.getApellidos() != null) { // es una persona física
            contenido = new Paragraph("He recibido de " + arrendatario.getNombre() + " " + arrendatario.getApellidos() + " con NIF " + arrendatario.getDni() + " la "
                    + "cantidad de " + NumeroEnTexto.convertirNumeroATextoDouble(total, false) + " euros ( " + decimal.format(total) + " € ) en concepto de arrendamiento del local de mi propiedad, situado "
    ...
    private void rellenarTablaCentral() throws DocumentException {
        ...
        PdfPCell[] arrayCell = new PdfPCell[8];
        String porcentajeRetencion = String.valueOf(decimal.format(configuracion.getIrpf() * 100));
        String porcentajeIgic = String.valueOf(decimal.format(configuracion.getIgic() * 100));
        arrayCell[0] = new PdfPCell(new Phrase("Renta"));
        arrayCell[1] = new PdfPCell(new Phrase(decimal.format(contrato.getPrecio1Inmueble()) + " €"));
        arrayCell[2] = new PdfPCell(new Phrase("Retención a/c IRPF (" + porcentajeRetencion + "%)"));
        arrayCell[3] = new PdfPCell(new Phrase(decimal.format(retencion) + " €"));
        arrayCell[4] = new PdfPCell(new Phrase("IGIC (" + porcentajeIgic + "%)"));
        arrayCell[5] = new PdfPCell(new Phrase(decimal.format(igic) + " €"));
        arrayCell[6] = new PdfPCell(new Phrase("Total"));
        arrayCell[7] = new PdfPCell(new Phrase(decimal.format(total) + " €"));
    ...
    private void calcularPrecioNeto() {
        ...
        this.retencion = precio1 * configuracion.getIrpf();
        this.igic = precio1 * configuracion.getIgic();
        this.total = precio1 - retencion + igic;
    }
}