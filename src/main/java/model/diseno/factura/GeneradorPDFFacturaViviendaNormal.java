...
public class GeneradorPDFFacturaViviendaNormal extends GeneradorPDFFactura {

    private Document documento;
    private DecimalFormat decimal;

    public GeneradorPDFFacturaViviendaNormal(Arrendatario arrendatario, Inmueble inmueble, Contrato contrato, Configuracion configuracion, Boolean version) {
    ...
    private void rellenarContenido(LocalDate fechaFactura) throws DocumentException {
        ...
        if (arrendatario.getApellidos() != null) { // es una persona física
            contenido = new Paragraph("He recibido de " + arrendatario.getNombre() + " " + arrendatario.getApellidos() + " con NIF " + arrendatario.getDni() + " la "
                    + "cantidad de " + NumeroEnTexto.convertirNumeroATextoDouble(contrato.getPrecio1Inmueble(), false) + " euros ( " + decimal.format(contrato.getPrecio1Inmueble()) + " € ) en concepto de arrendamiento de la vivienda de mi propiedad, situada "
